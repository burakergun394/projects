...
namespace PersonnelTransportAutomation.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("PersonnelTransportAutomation") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "PersonnelTransportAutomation", DisplayName = "PersonnelTransportAutomation API", Resources = { "PersonnelTransportAutomation" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        var commonScopes = new List<string> {
            ...
            "PersonnelTransportAutomation"
        };

        var configurationSection = _configuration.GetSection("OpenIddict:Applications");

        //Web Client
        var webClientId = configurationSection["PersonnelTransportAutomation_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["PersonnelTransportAutomation_Web:RootUrl"]!.EnsureEndsWith('/');

            /* PersonnelTransportAutomation_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["PersonnelTransportAutomation_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["PersonnelTransportAutomation_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["PersonnelTransportAutomation_Swagger:RootUrl"]?.TrimEnd('/');
