...
namespace PersonelTransportAutomation.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("PersonelTransportAutomation") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "PersonelTransportAutomation", DisplayName = "PersonelTransportAutomation API", Resources = { "PersonelTransportAutomation" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        var commonScopes = new List<string> {
            ...
            "PersonelTransportAutomation"
        };

        var configurationSection = _configuration.GetSection("OpenIddict:Applications");

        //Web Client
        var webClientId = configurationSection["PersonelTransportAutomation_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["PersonelTransportAutomation_Web:RootUrl"]!.EnsureEndsWith('/');

            /* PersonelTransportAutomation_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["PersonelTransportAutomation_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["PersonelTransportAutomation_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["PersonelTransportAutomation_Swagger:RootUrl"]?.TrimEnd('/');
